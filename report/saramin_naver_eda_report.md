...
## VI. 5대 직무별 필수 요구사항 vs 우대사항 TF-IDF 및 워드클라우드 세부 심층 EDA (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

본 섹션은 사람인 채용공고 5,000건 데이터셋(`recruit_processed.db`)에서 5대 직무별 공고 제목(title)과 필수 요건(requirement), 우대 사항(preferential) 텍스트를 공백으로 결합한 후, **학력 용어, 서술어, 신분용어, 잡음 불용어(대학졸업, 고등학교졸업이상, 지원가능, finance, and 등)를 완전 제거**하고, **TF-IDF 수치에 따라 글자 크기(비선형 스케일링), 색상 채도 및 명도(HSL 동적 color_func)를 극명하게 차별화**하여 한눈에 중요도를 파악할 수 있도록 리팩토링한 **TF-IDF 상위 30개 키워드 서브플롯**과 **워드클라우드 서브플롯** 분석 결과입니다.

### 기획/전략 직무: 필수 요구사항 vs 우대사항 텍스트 심층 분석 (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

![기획/전략 TF-IDF & WordCloud](../images/plot11_plan_tfidf_wordcloud.png)

#### [필수 자격 요건 TOP 10 TF-IDF 가중치 테이블]
| 순위 | 필수 키워드 | TF-IDF 가중치 |
|:---:|:---|:---:|
| 1 | 사업 | 84.50 |
| 2 | 사업기획 | 68.36 |
| 3 | 문서 | 59.68 |
...
#### [우대사항 TOP 10 TF-IDF 가중치 테이블]
| 순위 | 우대 키워드 | TF-IDF 가중치 |
|:---:|:---|:---:|
| 1 | 프로젝트 | 64.83 |
| 2 | 사업 | 61.67 |
| 3 | 글로벌 | 56.66 |
...
#### [기획/전략 데이터 분석 인사이트]
TF-IDF 가중치에 따른 비선형 파워 스케일링 및 HSL 동적 명도/채도 차별화를 적용한 결과, **[기획/전략]** 직무에서 가장 압도적인 가중치를 갖는 **사업, 사업기획, 문서, 역량, 프로젝트** 키워드가 어둡고 진한 고채도 대형 글씨로 서류 합격의 핵심 임계값을 선명히 나타내며, 우대 영역에서는 **프로젝트, 사업, 글로벌, 영어, 활용** 기술이 고대비로 면접 우대 가산점의 핵심 가치를 보여줍니다.

---

### 인사/노무 직무: 필수 요구사항 vs 우대사항 텍스트 심층 분석 (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

![인사/노무 TF-IDF & WordCloud](../images/plot12_hr_tfidf_wordcloud.png)
...
#### [인사/노무 데이터 분석 인사이트]
TF-IDF 가중치에 따른 비선형 파워 스케일링 및 HSL 동적 명도/채도 차별화를 적용한 결과, **[인사/노무]** 직무에서 가장 압도적인 가중치를 갖는 **인사, hr, 총무, 인사총무, 회계** 키워드가 어둡고 진한 고채도 대형 글씨로 서류 합격의 핵심 임계값을 선명히 나타내며, 우대 영역에서는 **인사, hr, 총무, 회계, 인사총무** 기술이 고대비로 면접 우대 가산점의 핵심 가치를 보여줍니다.

---

### 회계/재무 직무: 필수 요구사항 vs 우대사항 텍스트 심층 분석 (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

![회계/재무 TF-IDF & WordCloud](../images/plot13_acc_tfidf_wordcloud.png)
...
#### [회계/재무 데이터 분석 인사이트]
TF-IDF 가중치에 따른 비선형 파워 스케일링 및 HSL 동적 명도/채도 차별화를 적용한 결과, **[회계/재무]** 직무에서 가장 압도적인 가중치를 갖는 **회계, 세무, 재무, 경력직, 재무회계** 키워드가 어둡고 진한 고채도 대형 글씨로 서류 합격의 핵심 임계값을 선명히 나타내며, 우대 영역에서는 **회계, 자격증, 세무, 경력직, 재무** 기술이 고대비로 면접 우대 가산점의 핵심 가치를 보여줍니다.

---

### 마케팅 직무: 필수 요구사항 vs 우대사항 텍스트 심층 분석 (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

![마케팅 TF-IDF & WordCloud](../images/plot14_mkt_tfidf_wordcloud.png)
...
#### [마케팅 데이터 분석 인사이트]
TF-IDF 가중치에 따른 비선형 파워 스케일링 및 HSL 동적 명도/채도 차별화를 적용한 결과, **[마케팅]** 직무에서 가장 압도적인 가중치를 갖는 **마케팅, 마케터, 브랜드, 콘텐츠, 홍보** 키워드가 어둡고 진한 고채도 대형 글씨로 서류 합격의 핵심 임계값을 선명히 나타내며, 우대 영역에서는 **마케팅, 마케터, 브랜드, 콘텐츠, 글로벌** 기술이 고대비로 면접 우대 가산점의 핵심 가치를 보여줍니다.

---

### 데이터분석가/AI엔지니어 직무: 필수 요구사항 vs 우대사항 텍스트 심층 분석 (TF-IDF 수치별 글자크기/채도/명도 극명 차별화)

![데이터분석가/AI엔지니어 TF-IDF & WordCloud](../images/plot15_dev_tfidf_wordcloud.png)
...
#### [데이터분석가/AI엔지니어 데이터 분석 인사이트]
TF-IDF 가중치에 따른 비선형 파워 스케일링 및 HSL 동적 명도/채도 차별화를 적용한 결과, **[데이터분석가/AI엔지니어]** 직무에서 가장 압도적인 가중치를 갖는 **개발자, ai, java, 백엔드, 설계** 키워드가 어둡고 진한 고채도 대형 글씨로 서류 합격의 핵심 임계값을 선명히 나타내며, 우대 영역에서는 **개발자, ai, 프로젝트, 시스템, 백엔드** 기술이 고대비로 면접 우대 가산점의 핵심 가치를 보여줍니다.
